( window.p15r = window.p15r || []).push({
  ...
  callback: function (pid, paramObj, cfg) {
    ...
    if (cfg.sampleRate) {
      LUX.samplerate = cfg.sampleRate
    }
    processingWrapper.util.loadScript({
      url: cfg.url || `https://cdn.speedcurve.com/js/lux.js?id=${cfg.accountId}`,
      asyncMode: true,
      defer: true
    })
  },
  extra: {
    onRegister: function (config) {
      if (config.load === 'direct' && (config.url) !== undefined) {
        processingWrapper.execute(this.processingId, config)
      }
    },
    disable: function () {
      processingWrapper.log("disabling SpeedCurve not yet implemented");
    },
    enable: function () {
      processingWrapper.log("enabling SpeedCurve not yet implemented");
    },
    runOnce: true,
  },
  isCentral: true
